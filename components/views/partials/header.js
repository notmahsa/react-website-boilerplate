...
class Header extends React.Component { // eslint-disable-line react/prefer-stateless-function
  ...
  render() {
    ...
    if (!header || !nav) {
      return <div>Header: Received null props</div>;
    }
    return (
      <div>
        <div className="container text-center">
          <a href="/"><img src={LookupItemInMetadata(header.metadata, 'logo').value} alt="" height="60px;"/></a>
          <div className="primary">{LookupItemInMetadata(header.metadata, 'site_title').value}</div>
        </div>
        <nav className="navbar navbar-default">
          <div className="container-fluid" style={{marginLeft: '25%'}}>
            <div className="navbar-header">
              <div className="container">
                <button type="button" className="navbar-toggle" data-toggle="collapse" data-target="#siteNav">
