...
class ContactPage extends React.Component {
  ...
  validateForm = () => {
    let check = true;
    let form_validation = this.state.form_validation
  ...
  submitForm = async () => {
    ...
    if (this.validateForm()) {
      let response = await Request.contactForm(this.state.form, this.state.contact_form);
      if (response.status) {
    ...
    return (
      <Meta>
        <Head>
          ...
          <link rel="icon" type="image/png" href={`${this.state.header.metadata.favicon.imgix_url}?w=32`}
                sizes="32x32"/>
          <link rel="icon" type="image/png" href={`${this.state.header.metadata.favicon.imgix_url}?w=16`}
                sizes="16x16"/>
        </Head>
        <Header header={this.state.header} nav={this.state.nav}/>
        <Contact formStatus={this.state.form_status} handleChange={this.handleChange} submitForm={this.submitForm}
                 form={this.state.form} formValidation={this.state.form_validation}
